...
public class BlockTransformationUpgradeWrapper extends UpgradeWrapperBase<BlockTransformationUpgradeWrapper, BlockTransformationUpgradeItem> {
	...
	public void setResult(ItemStack result) {
		if (result.isEmpty()) {
			upgrade.sophisticatedFabricLibrary_remove(ModCoreDataComponents.RESULT_ITEM);
			return;
		}

		upgrade.sophisticatedFabricLibrary_set(ModCoreDataComponents.RESULT_ITEM, SimpleItemContent.copyOf(result));
		save();
	}

	public Optional<SimpleItemContent> getResult() {
		return Optional.ofNullable(upgrade.sophisticatedFabricLibrary_get(ModCoreDataComponents.RESULT_ITEM));
	}
