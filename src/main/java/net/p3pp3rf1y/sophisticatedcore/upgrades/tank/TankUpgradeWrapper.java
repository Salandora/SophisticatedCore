...
public class TankUpgradeWrapper extends UpgradeWrapperBase<TankUpgradeWrapper, TankUpgradeItem>
	...
	private boolean isValidFluidItem(ItemStack stack, boolean isOutput) {
		return stack.sophisticatedFabricLibrary_getCapability(Capabilities.FluidHandler.ITEM).map(fluidHandler ->
	...
	public void tick(@Nullable Entity entity, Level world, BlockPos pos) {
		...
		inventory.getStackInSlot(INPUT_SLOT).sophisticatedFabricLibrary_getCapability(Capabilities.FluidHandler.ITEM).ifPresent(fluidHandler ->
				didSomething.set(drainHandler(fluidHandler, stack -> inventory.setStackInSlot(INPUT_SLOT, stack)))
		);
		inventory.getStackInSlot(OUTPUT_SLOT).sophisticatedFabricLibrary_getCapability(Capabilities.FluidHandler.ITEM).ifPresent(fluidHandler ->
