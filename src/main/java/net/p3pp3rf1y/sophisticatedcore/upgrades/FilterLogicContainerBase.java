		...
		for (String key : data.getAllKeys()) {
			switch (key) {
				...
				case DATA_ADD_TAG_NAME -> {
					addTagName(TagKey.create(Registry.ITEM.key(), new ResourceLocation(data.getString(DATA_ADD_TAG_NAME))));
					return true;
				}
				case DATA_REMOVE_TAG_NAME -> {
					removeSelectedTag(TagKey.create(Registry.ITEM.key(), new ResourceLocation(data.getString(DATA_REMOVE_TAG_NAME))));
					return true;
				}
