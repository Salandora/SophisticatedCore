...
public abstract class JukeboxUpgradeTab extends UpgradeSettingsTab<JukeboxUpgradeContainer> {
	...
	protected void moveSlotsToTab() {
		...
		for (Slot discSlot : getContainer().getSlots()) {
			discSlot.x = x - screen.sophisticatedFabricLibrary_getGuiLeft() + 4 + (slotIndex % slotsInRow) * 18;
			discSlot.y = y - screen.sophisticatedFabricLibrary_getGuiTop() + TOP_Y + 1 + (slotIndex / slotsInRow) * 18;
			slotIndex++;
		}
	}
