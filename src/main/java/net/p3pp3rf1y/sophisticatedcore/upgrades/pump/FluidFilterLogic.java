...
public class FluidFilterLogic {
	...
	private void deserializeFluidFilters() {
		List<SimpleFluidContent> deserializedFilters = upgrade.sophisticatedFabricLibrary_getOrDefault(ModCoreDataComponents.FLUID_FILTERS, Collections.emptyList());
	...
	private void serializeFluidFilters() {
		upgrade.sophisticatedFabricLibrary_set(ModCoreDataComponents.FLUID_FILTERS, fluidFilters.stream().map(SimpleFluidContent::copyOf).toList());
	}
}
